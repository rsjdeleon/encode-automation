import sqlite3
from core.paths import CONFIG_DB_PATH
...
def init_db_config():
    with sqlite3.connect(DB_NAME_CONFIG) as conn:
        ...
        c.execute("CREATE INDEX IF NOT EXISTS idx_config_lists_key ON config_lists(key)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_config_items_list_order ON config_items(list_id, sort_order)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_config_items_list_col1 ON config_items(list_id, col1)")
        conn.commit()
...
def run_startup_config_migrations():
    """Fast-path startup migration runner.

    Applies the same migration intent as the individual migrate_* functions but
    in a single DB transaction/connection to reduce startup overhead.
    """
    with sqlite3.connect(DB_NAME_CONFIG) as conn:
        c = conn.cursor()

        def _list_row(key):
            c.execute("SELECT id, kind, col2_label FROM config_lists WHERE key = ?", (key,))
            return c.fetchone()

        def _migrate_list_to_pairs_tx(key, col1_label, col2_label):
            row = _list_row(key)
            if not row:
                return
            list_id, kind, _col2_label = row
            if kind != "pairs":
                c.execute(
                    "UPDATE config_lists SET kind = 'pairs', col1_label = ?, col2_label = ? WHERE id = ?",
                    (col1_label, col2_label, list_id),
                )
            c.execute("UPDATE config_items SET col2 = col1 WHERE list_id = ? AND col2 IS NULL", (list_id,))

        def _migrate_list_to_dual_pairs_tx(key):
            row = _list_row(key)
            if not row:
                return
            list_id, kind, _col2_label = row
            ...
            c.execute("UPDATE config_items SET extra2 = col2 WHERE list_id = ? AND extra2 IS NULL", (list_id,))

        # Backfill old list_of_city rows that predated col2/extra2.
        city_row = _list_row("list_of_city")
        if city_row:
            c.execute(
                "UPDATE config_items SET col2 = ?, extra2 = ? "
                "WHERE list_id = ? AND (col2 IS NULL OR extra2 IS NULL)",
                ("NCR (National Capital Region)", "NCR [National Capital Region]", city_row[0]),
            )

        # list -> pairs upgrades.
        _migrate_list_to_pairs_tx("gender_list", "Label", "Value")
        _migrate_list_to_pairs_tx("target_sector_list", "Label", "Value")
        _migrate_list_to_pairs_tx("mode_of_release", "Label", "Value")
        _migrate_list_to_pairs_tx("financial_assistance_list", "Label", "Value")
        _migrate_list_to_pairs_tx("civil_status_list", "Choice", "Mapped Value")
        _migrate_list_to_pairs_tx("fund_source_list", "Code", "Display Name")
        _migrate_list_to_pairs_tx("relationship_list", "Choice", "Mapped Value")
        _migrate_list_to_pairs_tx("client_sub_category", "Key", "Value")

        # approved_by_list special reshape: old col1 -> new col2, old col2 -> extra2.
        approved_row = _list_row("approved_by_list")
        if approved_row and approved_row[1] != "dual_pairs":
            approved_list_id = approved_row[0]
            c.execute(
                "UPDATE config_lists SET kind = 'dual_pairs', col1_label = 'Label', col2_label = 'GForm Value' "
                "WHERE id = ?",
                (approved_list_id,),
            )
            c.execute("UPDATE config_items SET extra2 = col2, col2 = col1 WHERE list_id = ?", (approved_list_id,))

        # pairs -> dual_pairs upgrades.
        for key in (
            "target_sector_list",
            "gender_list",
            "civil_status_list",
            ...
            _migrate_list_to_dual_pairs_tx(key)

        # region list special dual_pairs defaults.
        region_row = _list_row("region_list")
        if region_row:
            region_list_id, region_kind, _region_col2 = region_row
            if region_kind != "dual_pairs":
                c.execute(
                    "UPDATE config_lists SET kind = 'dual_pairs', col2_label = 'GForm Value' WHERE id = ?",
                    (region_list_id,),
                )
            c.execute(
                "UPDATE config_items SET col2 = ?, extra2 = ? "
                "WHERE list_id = ? AND (col2 IS NULL OR extra2 IS NULL)",
                ("NCR (National Capital Region)", "NCR [National Capital Region]", region_list_id),
            )

        # province.extra: region label -> region item id.
        c.execute("SELECT id FROM config_lists WHERE key = 'region_list'")
        region_row = c.fetchone()
        c.execute("SELECT id FROM config_lists WHERE key = 'province_list'")
        province_row = c.fetchone()
        if region_row and province_row:
            region_list_id, province_list_id = region_row[0], province_row[0]
            c.execute("SELECT id, col1 FROM config_items WHERE list_id = ?", (region_list_id,))
            region_id_by_label = {label: item_id for item_id, label in c.fetchall()}
            c.execute("SELECT id, extra FROM config_items WHERE list_id = ?", (province_list_id,))
            for item_id, extra in c.fetchall():
                if extra in region_id_by_label:
                    c.execute("UPDATE config_items SET extra = ? WHERE id = ?", (str(region_id_by_label[extra]), item_id))

        # city.extra: province label -> province item id.
        c.execute("SELECT id FROM config_lists WHERE key = 'province_list'")
        province_row = c.fetchone()
        c.execute("SELECT id FROM config_lists WHERE key = 'list_of_city'")
        city_row = c.fetchone()
        if province_row and city_row:
            province_list_id, city_list_id = province_row[0], city_row[0]
            c.execute("SELECT id, col1 FROM config_items WHERE list_id = ?", (province_list_id,))
            province_id_by_label = {label: item_id for item_id, label in c.fetchall()}
            c.execute("SELECT id, extra FROM config_items WHERE list_id = ?", (city_list_id,))
            for item_id, extra in c.fetchall():
                if extra in province_id_by_label:
                    c.execute("UPDATE config_items SET extra = ? WHERE id = ?", (str(province_id_by_label[extra]), item_id))

        # barangay.extra: city label/value -> city item id.
        c.execute("SELECT id FROM config_lists WHERE key = 'list_of_city'")
        city_row = c.fetchone()
        c.execute("SELECT id FROM config_lists WHERE key = 'barangay_list'")
        barangay_row = c.fetchone()
        if city_row and barangay_row:
            city_list_id, barangay_list_id = city_row[0], barangay_row[0]
            c.execute("SELECT id, col1, col2 FROM config_items WHERE list_id = ?", (city_list_id,))
            city_id_by_label = {}
            for item_id, col1, col2 in c.fetchall():
            ...
            for item_id, extra in c.fetchall():
                ...
                city_id = city_id_by_label.get(extra.strip().upper())
                if city_id is not None:
                    c.execute("UPDATE config_items SET extra = ? WHERE id = ?", (str(city_id), item_id))

        # province dual-pairs normalization.
        province_row = _list_row("province_list")
        if province_row:
            province_list_id, _province_kind, province_col2_label = province_row
            if province_col2_label != "GForm Value":
                c.execute("UPDATE config_lists SET col2_label = 'GForm Value' WHERE id = ?", (province_list_id,))
            c.execute(
                "UPDATE config_items SET col2 = COALESCE(col2, col1), extra2 = COALESCE(extra2, col1) "
                "WHERE list_id = ? AND (col2 IS NULL OR extra2 IS NULL)",
                (province_list_id,),
        ...
        # city dual-pairs normalization.
        city_row = _list_row("list_of_city")
        if city_row:
            OLD_REGION_GFORM = "NCR (National Capital Region)"
            OLD_REGION_WEBSITE = "NCR [National Capital Region]"
            city_list_id, _city_kind, city_col2_label = city_row
            if city_col2_label != "GForm Value":
                c.execute("UPDATE config_lists SET col2_label = 'GForm Value' WHERE id = ?", (city_list_id,))
            c.execute("SELECT id, col1, col2, extra2 FROM config_items WHERE list_id = ?", (city_list_id,))
            for item_id, col1, col2, extra2 in c.fetchall():
                new_col2 = col1 if col2 in (None, OLD_REGION_GFORM) else col2
                default_website = "KALOOKAN CITY" if col1 == "CITY OF CALOOCAN" else col1
                new_extra2 = default_website if extra2 in (None, OLD_REGION_WEBSITE) else extra2
                if new_col2 != col2 or new_extra2 != extra2:
                    c.execute("UPDATE config_items SET col2 = ?, extra2 = ? WHERE id = ?", (new_col2, new_extra2, item_id))

        # barangay dual-pairs normalization.
        barangay_row = _list_row("barangay_list")
        if barangay_row:
            barangay_list_id, _barangay_kind, barangay_col2_label = barangay_row
            if barangay_col2_label != "GForm Value":
                c.execute("UPDATE config_lists SET col2_label = 'GForm Value' WHERE id = ?", (barangay_list_id,))
            c.execute(
                "UPDATE config_items SET col2 = COALESCE(col2, col1), extra2 = COALESCE(extra2, col1) "
                "WHERE list_id = ? AND (col2 IS NULL OR extra2 IS NULL)",
                (barangay_list_id,),
        ...
        # Cleanup legacy list key if present.
        c.execute("SELECT id FROM config_lists WHERE key = 'approver_list'")
        row = c.fetchone()
        if row:
            list_id = row[0]
            c.execute("DELETE FROM config_items WHERE list_id = ?", (list_id,))
